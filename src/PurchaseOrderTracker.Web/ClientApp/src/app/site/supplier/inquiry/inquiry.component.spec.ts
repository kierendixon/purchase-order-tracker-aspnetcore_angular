...
import { SupplierTestHelper } from '../../../../test/supplier-test-helper';
...
describe('InquiryComponent', () => {
  ...
  describe('#hasRecords', () => {
    ...
    it("returns false if component's model has no items", () => {
      component.model = SupplierTestHelper.buildInquiryResult();
      expect(component.hasRecords()).toBe(false);
    });

    it("returns true if component's model has items", () => {
      component.model = SupplierTestHelper.buildInquiryResultWithItemsCount(1);
