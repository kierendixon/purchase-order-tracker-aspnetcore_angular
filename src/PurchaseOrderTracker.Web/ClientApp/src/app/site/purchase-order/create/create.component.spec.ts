...
describe('CreateComponent', () => {
  ...
  describe('#ngOnInit', () => {
    it("updates component's model with response", () => {
      ...
      const handleSuppliersQuerySpy = spyOn(createService, 'handleSuppliersQuery').and.returnValue(
        of(suppliersQueryResult)
      );
    ...
    it('sends error to messsage service if error returned', () => {
      ...
      const createService = fixture.debugElement.injector.get(CreateService);
      const handleSuppliersQuerySpy = spyOn(createService, 'handleSuppliersQuery').and.returnValue(throwError(error));
