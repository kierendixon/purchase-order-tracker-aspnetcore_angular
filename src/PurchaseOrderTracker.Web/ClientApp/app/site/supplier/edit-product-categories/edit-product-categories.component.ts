...
export class EditProductCategoriesComponent implements OnInit {
    ...
    refreshData(): void {
        let query = new EditProductCategoriesQuery(this.supplierId, this.pageNumber);
    ...
    onDeleteCategory(index: number): void {
        let category = this.model.items[index];
        let command = new DeleteCommand(this.supplierId, category.id);
    ...
    onSubmitEditCategory(index: number): void {
        let category = this.model.items[index];
        let command = new EditProductCategoryCommand(this.supplierId, category.id, category.name);
