...
export class InquiryComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.route.queryParams.subscribe(params => {
            let queryType = params[queryTypeQueryParam];
            let pageNumber = params[pageNumberQueryParam] == undefined ? this.defaultPageNumber : Number(params[pageNumberQueryParam]);
            this.refreshData(queryType, pageNumber);
        });
    }

    refreshData(queryType: string, pageNumber: number): void {
        let query = new InquiryQuery(queryType, pageNumber);
